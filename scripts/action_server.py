...
class RobotActionsServer:
    ...
    def __init__(self, object_dict, root_path, random_seed=10):
        ...
        self.terminal_state = {'x': 0.0, 'y': 0.0, 'orientation': 'EAST'}
        self.maze_dim = (0, 0)
        self.maze = copy.deepcopy(self.load_maze(root_path + '/sample_maze.mz'))
        # self.check_successors()  # Uncomment to verify the response
    ...
    def generate_init_state(self):
        state = {}
        state = {'x': 0.0, 'y': 0.0, 'orientation': 'EAST'}
    ...
    def load_maze(self, maze_file):
        ...
        f.seek(0)
        n = len(f.readlines())
        self.maze_dim = (n , m)  # n rows m columns
        self.maze = arr = [[0 for i in range(m)] for j in range(n)]
        f.seek(0)
        t = (0,0)
        cnt = 0
        print("maze_dim: ", self.maze_dim)
        for i in range(0, n):
            for j in range(0, m + 1):
                ...
                if c == '|' or c =='_':
                    ...
                    print("path at ", i, j)
                    t = (i, j)
        self.terminal_state = {'x': (t[0])/2.0, 'y': (t[1])/2.0, 'orientation': 'EAST'}
    ...
    def check_successors(self):
        ...
        for row in self.maze:
            print(row)
        for i in range(0, self.maze_dim[0]):
            for j in range(0, self.maze_dim[1]):
                print()
                cs = {"x":i/2.0,"y":j/2.0,"orientation":"EAST"}
                self.get_successors(cs)
                cs = {"x":i/2.0,"y":j/2.0,"orientation":"NORTH"}
                self.get_successors(cs)
                cs = {"x":i/2.0,"y":j/2.0,"orientation":"SOUTH"}
                self.get_successors(cs)
                cs = {"x":i/2.0,"y":j/2.0,"orientation":"WEST"}
                self.get_successors(cs)

    def get_successors(self, current_state):
        x1 = current_state["x"] * 2
        y1 = current_state["y"] * 2
        x2, y2 = 0, 0
        ...
        successors = []
        cnt = 0
        print("elf x1, x2", x1, x2)
        if self.maze[int(x1)][int(y1)] == 0 :
            if "EAST" in ori:
                x2 = x1 + 1
                y2 = y1
                ori_cw = "SOUTH"
            ...
            if x1 == 0 and ori == "WEST":
                print("case 1 in x1, y1", x1, y1)
                successors.append({"state":{"x":x2/2.0,"y":y2/2.0,"orientation":ori}, "action": "MoveF", "cost" : 1 * self.wall_cost})
            elif x2 >= 0 and x2 < self.maze_dim[0] and y2 >= 0 and y2 < self.maze_dim[1]:
                successors.append({"state":{"x":x2/2.0,"y":y2/2.0,"orientation":ori}, "action": "MoveF", "cost" : self.maze[int(x2)][int(y2)] * self.wall_cost})
            else:
                print("Invalid state ", current_state)
                return {}
            successors.append({"state":{"x":current_state["x"],"y":current_state["y"],"orientation":ori_cw}, "action": "TurnCW", "cost" : self.turn_cost})
            successors.append({"state":{"x":current_state["x"],"y":current_state["y"],"orientation":ori_ccw}, "action": "TurnCCW", "cost" : self.turn_cost})
            print("successors for state: ", current_state, " length ", len(successors))
            print(successors)
        else:
            print("wall at x1,y1", x1, y1)
        return successors
    ...
    def is_terminal_state(self, state):
        ...
        cnt = 0
        if self.terminal_state.x == state["robot"].x and self.terminal_state.y == state["robot"].y:
            return 1
        return 0
