...
class Maze:
    ...
    def generate_maze(self, grid_dimension, n_obstacles, seed, scale=0.5):
        ...
        with open('sample_maze.mz', 'r') as mz:
            lines = mz.read().splitlines()
        matrix = []
        for i, j in enumerate(lines):
            matrix.append(list(j))
        grid_dimension = len(matrix)
        ...
        for i in range(len(matrix)):
            for j in range(len(matrix[0])):
                x,y=i*scale,j*scale
                if (i ==0 and j == 1) or (i==1 and j == 0) or (i==0 and j==0) or (i==sz-1 and j==sz-2) or (i==sz-2 and j==sz-1) or ( i==sz-1 and j== sz-1):
                    continue
                if i == 0 and matrix[i][j] == ' ':
                    coords.append((x-0.2, y))
                    self.add_can(f_out, x-0.2, y)
                    blocked_edges.add((x-0.2, y-scale, x-0.2, y+scale))
                    hv.append(0)
                if i == sz-1 and matrix[i][j] == ' ':
                    coords.append((x +0.5 , y))
                    self.add_can(f_out, x +0.5 , y)
                    blocked_edges.add((x +0.5, y - scale, x +0.5, y + scale))
                    hv.append(0)
                if matrix[i][j] == '|':
                    coords.append((x, y))
                    self.add_can(f_out, x, y)
                    blocked_edges.add((x-scale, y, x+scale, y))
                    hv.append(1)
                elif matrix[i][j] == '_':
                    coords.append((x, y))
                    blocked_edges.add((x, y-scale, x, y+scale))
                    self.add_can(f_out, x, y)
                    hv.append(0)
...
#			count+=1
            ...
            if(flag == 0 and ((x+scale) <= grid_dimension*scale) and ((x, y, x+scale, y) not in blocked_edges)):
                ...
                offset = 0#np.random.uniform(0, 0.07*scale)
        ...
        self.add_can_description(f_out, coords, hv)
        f_out.write('</world>\n</sdf>')
        f_out.close()
